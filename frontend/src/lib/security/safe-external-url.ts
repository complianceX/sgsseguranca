...
const BACKBLAZE_HOST_SUFFIX = ".backblazeb2.com";
...
function isAllowedStorageHost(hostname: string) {
  return (
    hostname === "r2.cloudflarestorage.com" ||
    hostname.endsWith(R2_HOST_SUFFIX) ||
    hostname === "backblazeb2.com" ||
    hostname.endsWith(BACKBLAZE_HOST_SUFFIX)
  );
}
...
function isAllowedArtifactOrigin(parsed: URL, browserOrigin: string) {
  ...
  const isStorageOrigin =
    parsed.protocol === "https:" && isAllowedStorageHost(parsed.hostname);

  return isSameOrigin || isAppOrigin || isApiOrigin || isStorageOrigin;
}
