...
if (-not $SkipDocker) {
  if (-not (Test-Path './ops/docker/docker-compose.local.yml')) {
    Write-Host 'ops/docker/docker-compose.local.yml não encontrado; nada para parar no Docker.'
    exit 0
  }

  docker compose -f ops/docker/docker-compose.local.yml down
