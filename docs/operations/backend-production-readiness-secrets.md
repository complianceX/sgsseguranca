...
| ---------------------------------- | --------------- | -------------------------------------------------------------- | ------------------------------------------------ |
| `ADMIN_IP_ALLOWLIST_REQUIRED=true` | web             | Faz `/admin/*` falhar fechado se a allowlist estiver ausente   | dashboard mostra chave presente |
...
| `REDIS_AUTH_URL`                   | web/worker      | Sessao, refresh, blacklist e brute-force em Redis sem eviction | Redis `sgs-redis` na rede Docker `coolify`, `noeviction` |
| `REDIS_CACHE_URL`                  | web/worker      | Cache/rate-limit em Redis evictable                            | Redis `sgs-redis` na rede Docker `coolify`       |
| `REDIS_QUEUE_URL`                  | web/worker      | BullMQ sem eviction                                            | Redis `sgs-redis` na rede Docker `coolify`, `noeviction` |
| `ANTIVIRUS_PROVIDER=clamav`        | web/worker      | Uploads governados falham fechado em producao                  | Health/log mostra provider ativo                 |
| `CLAMAV_HOST=sgs-clamav-internal`  | web/worker      | Liga runtime ao servico ClamAV privado                         | Confirmar DNS interno                  |
...
O contrato operacional agora separa Redis em tres conexoes logicas. Na
infraestrutura atual, todas sao atendidas pelo container self-hosted `sgs-redis`,
na mesma VPS Hostinger dos apps Coolify e restrito à rede Docker `coolify`:

- `REDIS_AUTH_URL`: sessao, refresh, blacklist e brute-force.
- `REDIS_CACHE_URL`: cache e rate-limit recalculavel.
- `REDIS_QUEUE_URL`: BullMQ sem eviction.
