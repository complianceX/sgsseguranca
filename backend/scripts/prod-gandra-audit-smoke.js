/**
 * Smoke test controlado de emissão de Auditoria em produção (empresa Gandra).
 *
 * Fluxo validado:
 *   1. Cria Auditoria (POST /audits, titulo como marcador)
 *   2. Anexa direto (auditoria não exige status/assinatura prévia)
 *   3. Anexa PDF final governado (POST /audits/:id/file)
 *   4. Confere acesso ao PDF (GET /audits/:id/pdf) e estado do banco
 *      (audits + document_registry com hash)
 *
 * Observação: o módulo Auditoria não possui portal de validação pública com grant
...
const SMOKE_USER_NAME = 'K6 TESTE GANDRA AUDIT CONTROLADO';
const SMOKE_USER_EMAIL = 'k6.gandra.audit.smoke@invalid.local';
const SMOKE_USER_FUNCTION = 'TST Smoke Controlado';
const SMOKE_USER_BASE_CPF = '987654312';
const UA = 'sgs-prod-gandra-audit-smoke/1.0';
...
function buildSmokeTitulo() {
  ...
  return `${TEST_DOCUMENT_PREFIX}${stamp}-AUD-001`;
}
...
async function run() {
  ...
  try {
    ...
    if (resumable)
      warnings.push(
        `resuming_existing_audit:${maskId(resumable.id)} status=${resumable.status}`,
      );
    ...
    // 1. Criação
    let auditId = resumable?.id || null;
    let currentStatus = resumable?.status || null;
    let hasPdfAlready = Boolean(resumable?.pdf_file_key);

    if (!auditId) {
      const created = await requestJson('/audits', session.accessToken, {
        method: 'POST',
        companyId: TEST_COMPANY_ID,
        includeCsrf: true,
        ...
        body: JSON.stringify({
          ...
          site_id: site.id,
          auditor_id: smokeUser.userId,
          objetivo: 'Validação documental controlada em produção (smoke test).',
          escopo: 'Escopo sintético. Sem execução operacional real.',
        }),
      });
      if (!created.ok || !created.body?.id) {
        throw new Error(
          `Criação da auditoria falhou. status=${created.status} body=${JSON.stringify(created.body)}`,
        );
      }
    ...
    if (!hasPdfAlready) {
      ...
      warnings.push('audit_pdf_already_attached');
    }
    ...
    const db = await verifyDatabaseState(client, auditId);
    ...
    const report = {
      ...
      database: {
        auditId: maskId(db.audit.id),

        auditCompanyId: maskId(db.audit.company_id),
        siteCompanyId: maskId(db.audit.site_company_id),
        hasPdfKey: Boolean(db.audit.pdf_file_key),
      ...
      publicValidation: {
        status: null,
        valid: null,
        note: 'not_applicable: módulo Auditoria não possui portal de validação pública com grant.',
      },
      artifacts: {
