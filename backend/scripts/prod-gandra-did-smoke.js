/**
 * Smoke test controlado de emissão de DID em produção (empresa Gandra).
 *
 * Fluxo validado:
 *   1. Cria DID (POST /dids)
 *   2. Alinha o DID (PATCH /dids/:id/status → alinhado)
 *   3. Anexa PDF final governado (POST /dids/:id/file) — status vira "executado"
 *   4. Confere acesso ao PDF (GET /dids/:id/pdf) e estado do banco
 *      (dids + document_registry com hash)
 *
 * Observação: o módulo DID não possui portal de validação pública com grant
...
const SMOKE_USER_NAME = 'K6 TESTE GANDRA DID CONTROLADO';
const SMOKE_USER_EMAIL = 'k6.gandra.did.smoke@invalid.local';
const SMOKE_USER_FUNCTION = 'TST Smoke Controlado';
const SMOKE_USER_BASE_CPF = '987654318';
const UA = 'sgs-prod-gandra-did-smoke/1.0';
...
function buildSmokeTitulo() {
  ...
  return `${TEST_DOCUMENT_PREFIX}${stamp}-DID-001`;
}
...
async function findResumableDid(client, titulo) {
  ...
  if (rows.rows.length >= MAX_TEST_DOCUMENTS) {
    ...
    throw new Error(
      `Já existe ${rows.rows.length} DID de teste com prefixo ${TEST_DOCUMENT_PREFIX}. Abortando.`,
    );
  }
...
async function buildSmokePdfBuffer(params) {
  ...
  page.drawText('SGS - DID Smoke Test Controlado', {
    x: 52,
    y: 788,
    size: 18,
    font: bold,
...
async function attachPdf(accessToken, didId, pdfBuffer, fileName) {
  const csrfHeaders = await getMutationCsrfHeaders();
  const form = new FormData();
  form.set('file', new File([pdfBuffer], fileName, { type: 'application/pdf' }));
  const response = await fetch(`${API_BASE_URL}/dids/${didId}/file`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'User-Agent': UA,
...
async function verifyDatabaseState(client, didId) {
  const didRes = await client.query(
  ...
  if (didRes.rows.length !== 1)
    throw new Error('DID criado não encontrado para verificação final.');
  ...
  const registryRes = await client.query(
    `SELECT id, company_id, module, entity_id, document_code, file_key,
      ...
      WHERE module = 'did' AND entity_id = $1
      ORDER BY created_at DESC`,
    [didId],
  );

  const did = didRes.rows[0];
  if (did.company_id !== TEST_COMPANY_ID)
    throw new Error('DID persistido fora do tenant esperado.');
  if (did.site_company_id !== TEST_COMPANY_ID)
    throw new Error('Site do DID não pertence ao tenant esperado.');
  if (registryRes.rows.length !== 1)
    throw new Error(
      `Esperado 1 registro governado para o DID; encontrado ${registryRes.rows.length}.`,
    );
  if (registryRes.rows[0].company_id !== TEST_COMPANY_ID)
    throw new Error('Registro governado pertence a tenant incorreto.');

  return { did, registry: registryRes.rows[0] };
}
...
async function run() {
  ...
  try {
    ...
    if (resumable)
      warnings.push(
        `resuming_existing_did:${maskId(resumable.id)} status=${resumable.status}`,
      );
    ...
    if (!didId) {
      const created = await requestJson('/dids', session.accessToken, {
        ...
        body: JSON.stringify({
          ...
          atividades_planejadas:
            'Validação documental controlada em produção. Sem execução operacional real.',
          riscos_operacionais:
            'Risco sintético para validação documental (sem execução real).',
      ...
      if (!created.ok || !created.body?.id) {
        throw new Error(
          `Criação do DID falhou. status=${created.status} body=${JSON.stringify(created.body)}`,
        );
      }
      didId = created.body.id;
      currentStatus = created.body.status || 'rascunho';
    }

    // 2. Alinhar (rascunho → alinhado), pré-requisito do PDF final
    if (currentStatus === 'rascunho') {
      const aligned = await requestJson(
        ...
        session.accessToken,
        {
          ...
          includeCsrf: true,
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ status: 'alinhado' }),
        },
      );
      if (!aligned.ok) {
        throw new Error(
          `Alinhamento do DID falhou. status=${aligned.status} body=${JSON.stringify(aligned.body)}`,
        );
      }
      currentStatus = 'alinhado';
    }
    ...
    if (!hasPdfAlready) {
      ...
      warnings.push('did_pdf_already_attached');
    }
    ...
    const db = await verifyDatabaseState(client, didId);
    ...
    const report = {
      ...
      database: {
        didId: maskId(db.did.id),
        didStatus: db.did.status,
        didCompanyId: maskId(db.did.company_id),
        siteCompanyId: maskId(db.did.site_company_id),
        hasPdfKey: Boolean(db.did.pdf_file_key),
      ...
      publicValidation: {
        status: null,
        valid: null,
        note: 'not_applicable: módulo DID não possui portal de validação pública com grant.',
      },
      artifacts: {
