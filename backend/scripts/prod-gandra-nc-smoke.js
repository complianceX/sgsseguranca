/**
 * Smoke test controlado de emissão de Não Conformidade em produção (empresa Gandra).
 *
 * Fluxo validado:
 *   1. Cria NC (POST /nonconformities, codigo_nc como marcador)
 *   2. Anexa direto (NC não exige status/assinatura prévia)
 *   3. Anexa PDF final governado (POST /nonconformities/:id/file)
 *   4. Confere acesso ao PDF (GET /nonconformities/:id/pdf) e estado do banco
 *      (nonconformities + document_registry com hash)
 *
 * Observação: o módulo NC não possui portal de validação pública com grant
...
const SMOKE_USER_NAME = 'K6 TESTE GANDRA NC CONTROLADO';
const SMOKE_USER_EMAIL = 'k6.gandra.nc.smoke@invalid.local';
const SMOKE_USER_FUNCTION = 'TST Smoke Controlado';
const SMOKE_USER_BASE_CPF = '987654313';
const UA = 'sgs-prod-gandra-nc-smoke/1.0';
...
function buildSmokeTitulo() {
  ...
  return `${TEST_DOCUMENT_PREFIX}${stamp}-NC-001`;
}
...
async function findResumableDid(client, titulo) {
  const rows = await client.query(
    `SELECT id, codigo_nc, descricao, status, pdf_file_key
       FROM nonconformities
      WHERE company_id = $1 AND deleted_at IS NULL AND codigo_nc LIKE $2
  ...
  return rows.rows.find((row) => row.codigo_nc === titulo) || null;
}
...
async function buildSmokePdfBuffer(params) {
  ...
  page.drawText('SGS - NC Smoke Test Controlado', {
    x: 52,
    y: 788,
    size: 18,
    font: bold,
...
async function attachPdf(accessToken, ncId, pdfBuffer, fileName) {
  const csrfHeaders = await getMutationCsrfHeaders();
  const form = new FormData();
  form.set('file', new File([pdfBuffer], fileName, { type: 'application/pdf' }));
  const response = await fetch(`${API_BASE_URL}/nonconformities/${ncId}/file`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'User-Agent': UA,
...
async function verifyDatabaseState(client, ncId) {
  const ncRes = await client.query(
    `SELECT d.id, d.codigo_nc, d.status, d.company_id, d.site_id,
            d.pdf_file_key,
            s.company_id AS site_company_id
       FROM nonconformities d
       JOIN sites s ON s.id = d.site_id
      WHERE d.id = $1 AND d.deleted_at IS NULL`,
    [ncId],
  );
  if (ncRes.rows.length !== 1)
    throw new Error('NC criada não encontrada para verificação final.');
  ...
  const registryRes = await client.query(
    `SELECT id, company_id, module, entity_id, document_code, file_key,
      ...
      WHERE module = 'nonconformity' AND entity_id = $1
      ORDER BY created_at DESC`,
    [ncId],
  );

  const cat = ncRes.rows[0];
  if (cat.company_id !== TEST_COMPANY_ID)
    throw new Error('NC persistida fora do tenant esperado.');
  if (cat.site_company_id !== TEST_COMPANY_ID)
    throw new Error('Site da NC não pertence ao tenant esperado.');
  if (registryRes.rows.length !== 1)
    throw new Error(
      `Esperado 1 registro governado para a NC; encontrado ${registryRes.rows.length}.`,
    );
  if (registryRes.rows[0].company_id !== TEST_COMPANY_ID)
    throw new Error('Registro governado pertence a tenant incorreto.');

  return { nc: cat, registry: registryRes.rows[0] };
}
...
async function run() {
  ...
  try {
    ...
    if (resumable)
      warnings.push(
        `resuming_existing_nc:${maskId(resumable.id)} status=${resumable.status}`,
      );
    ...
    // 1. Criação
    let ncId = resumable?.id || null;
    let currentStatus = resumable?.status || null;
    let hasPdfAlready = Boolean(resumable?.pdf_file_key);

    if (!ncId) {
      const created = await requestJson('/nonconformities', session.accessToken, {
        method: 'POST',
        companyId: TEST_COMPANY_ID,
        includeCsrf: true,
        ...
        body: JSON.stringify({
          ...
          local_setor_area: 'Área de validação técnica (sintética)',
          atividade_envolvida: 'Atividade sintética de smoke test controlado',
          responsavel_area: 'TST Smoke Controlado',
          auditor_responsavel: 'TST Smoke Controlado',
          descricao:
            'Documento de teste controlado em produção. Desvio sintético sem ocorrência real. Dados sintéticos para validação documental.',
          evidencia_observada:
            'Evidência sintética registrada apenas para validação técnica.',
          condicao_insegura:
            'Condição insegura sintética (sem execução operacional real).',
          requisito_nr: 'NR-01',
          requisito_item: '1.4.1',
          risco_perigo: 'Perigo sintético de teste',
          risco_associado: 'Risco sintético de validação documental',
          risco_nivel: 'Baixo',
          status: 'ABERTA',
          observacoes_gerais: 'SMOKE TEST CONTROLADO. USO EXCLUSIVO DE VALIDACAO.',
          site_id: site.id,
        }),
      });
      if (!created.ok || !created.body?.id) {
        throw new Error(
          `Criação da NC falhou. status=${created.status} body=${JSON.stringify(created.body)}`,
        );
      }
      ncId = created.body.id;
      currentStatus = created.body.status || 'ABERTA';
    }

    // 2. Anexo do PDF final governado (NC não exige status/assinatura prévia)
    ...
    const db = await verifyDatabaseState(client, ncId);
    ...
    const report = {
      ...
      database: {
        ncId: maskId(db.nc.id),
        ncStatus: db.nc.status,
        ncCompanyId: maskId(db.nc.company_id),
        siteCompanyId: maskId(db.nc.site_company_id),
        hasPdfKey: Boolean(db.nc.pdf_file_key),
      ...
      publicValidation: {
        status: null,
        valid: null,
        note: 'not_applicable: módulo NC não possui portal de validação pública com grant.',
      },
      artifacts: {
