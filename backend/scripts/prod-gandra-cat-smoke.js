/**
 * Smoke test controlado de emissão de CAT em produção (empresa Gandra).
 *
 * Fluxo validado:
 *   1. Cria CAT (POST /cats, numero como marcador)
 *   2. Investiga (POST /cats/:id/investigation) e fecha (POST /cats/:id/close)
 *   3. Anexa PDF final governado (POST /cats/:id/pdf/file)
 *   4. Confere acesso ao PDF (GET /cats/:id/pdf) e estado do banco
 *      (cats + document_registry com hash)
 *
 * Observação: o portal cat_public_validation existe no registry, mas a API
 * não expõe validation-context para CATs — sem token, a validação pública
 * não pode ser exercitada pelo smoke (reportada como "partial").
 *
...
const SMOKE_USER_NAME = 'K6 TESTE GANDRA CAT CONTROLADO';
const SMOKE_USER_EMAIL = 'k6.gandra.cat.smoke@invalid.local';
const SMOKE_USER_FUNCTION = 'TST Smoke Controlado';
const SMOKE_USER_BASE_CPF = '987654314';
const UA = 'sgs-prod-gandra-cat-smoke/1.0';
...
function buildSmokeTitulo() {
  ...
  return `${TEST_DOCUMENT_PREFIX}${stamp}-CAT-001`;
}
...
async function findResumableDid(client, titulo) {
  const rows = await client.query(
    `SELECT id, numero, descricao, status, pdf_file_key
       FROM cats
      WHERE company_id = $1 AND deleted_at IS NULL AND numero LIKE $2
  ...
  return rows.rows.find((row) => row.numero === titulo) || null;
}
...
async function buildSmokePdfBuffer(params) {
  ...
  page.drawText('SGS - CAT Smoke Test Controlado', {
    x: 52,
    y: 788,
    size: 18,
    font: bold,
...
async function attachPdf(accessToken, catId, pdfBuffer, fileName) {
  const csrfHeaders = await getMutationCsrfHeaders();
  const form = new FormData();
  form.set('file', new File([pdfBuffer], fileName, { type: 'application/pdf' }));
  const response = await fetch(`${API_BASE_URL}/cats/${catId}/pdf/file`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'User-Agent': UA,
...
async function verifyDatabaseState(client, catId) {
  ...
  if (catRes.rows.length !== 1)
    throw new Error('CAT criada não encontrada para verificação final.');
  ...
  const registryRes = await client.query(
    `SELECT id, company_id, module, entity_id, document_code, file_key,
      ...
      WHERE module = 'cat' AND entity_id = $1
      ORDER BY created_at DESC`,
    [catId],
  );
  ...
  if (cat.company_id !== TEST_COMPANY_ID)
    throw new Error('CAT persistida fora do tenant esperado.');
  if (cat.site_company_id !== TEST_COMPANY_ID)
    throw new Error('Site da CAT não pertence ao tenant esperado.');
  if (registryRes.rows.length !== 1)
    throw new Error(
      `Esperado 1 registro governado para a CAT; encontrado ${registryRes.rows.length}.`,
    );
  if (registryRes.rows[0].company_id !== TEST_COMPANY_ID)
    throw new Error('Registro governado pertence a tenant incorreto.');

  return { cat, registry: registryRes.rows[0] };
}
...
async function run() {
  ...
  try {
    ...
    if (resumable)
      warnings.push(
        `resuming_existing_cat:${maskId(resumable.id)} status=${resumable.status}`,
      );
    ...
    if (!catId) {
      const created = await requestJson('/cats', session.accessToken, {
        ...
        body: JSON.stringify({
          ...
          gravidade: 'leve',
          descricao:
            'Documento de teste controlado em produção. Ocorrência sintética sem acidente real. Dados sintéticos para validação documental.',
          local_ocorrencia: 'Área de validação técnica (sintética)',
          acao_imediata:
            'Nenhuma ação real. Registro de smoke test controlado.',
          site_id: site.id,
        }),
      });
      if (!created.ok || !created.body?.id) {
        throw new Error(
          `Criação da CAT falhou. status=${created.status} body=${JSON.stringify(created.body)}`,
        );
      }
      catId = created.body.id;
      currentStatus = created.body.status || 'aberta';
    }

    // 2. Investigação (pré-requisito do fechamento)
    if (currentStatus !== 'fechada' && !hasPdfAlready) {
      const investigated = await requestJson(
        `/cats/${catId}/investigation`,
        session.accessToken,
        {
          method: 'POST',
          companyId: TEST_COMPANY_ID,
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            investigacao_detalhes:
              'Investigação sintética de smoke test controlado. Sem ocorrência real.',
            causa_raiz: 'Nenhuma — registro de validação documental.',
          }),
        },
      );
      if (!investigated.ok) {
        throw new Error(
          `Investigação da CAT falhou. status=${investigated.status} body=${JSON.stringify(investigated.body)}`,
        );
      }
      currentStatus = investigated.body?.status || currentStatus;
    }

    // 3. Fechamento (status vira "fechada", pré-requisito do PDF final)
    if (currentStatus !== 'fechada') {
      const closed = await requestJson(
        ...
        session.accessToken,
        {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            plano_acao_fechamento:
              'Fechamento sintético de smoke test controlado. Sem plano de ação real.',
            licoes_aprendidas:
              'Documento de validação técnica em produção.',
          }),
        },
      );
      if (!closed.ok) {
        throw new Error(
          `Fechamento da CAT falhou. status=${closed.status} body=${JSON.stringify(closed.body)}`,
        );
      }
    ...
    if (!hasPdfAlready) {
      ...
      warnings.push('cat_pdf_already_attached');
    }
    ...
    const db = await verifyDatabaseState(client, catId);
    ...
    const report = {
      ...
      database: {
        catId: maskId(db.cat.id),
        catStatus: db.cat.status,
        catCompanyId: maskId(db.cat.company_id),
        siteCompanyId: maskId(db.cat.site_company_id),
        hasPdfKey: Boolean(db.cat.pdf_file_key),
      ...
      publicValidation: {
        status: null,
        valid: null,
        note: 'partial: portal cat_public_validation existe, mas a API não expõe validation-context para CATs (token não emitido).',
      },
      artifacts: {
