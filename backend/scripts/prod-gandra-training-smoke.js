/**
 * Smoke test controlado de emissão de Treinamento em produção (empresa Gandra).
 *
 * Fluxo validado:
 *   1. Cria Treinamento (POST /trainings, nome como marcador)
 *   2. Anexa direto (treinamento não exige status/assinatura prévia)
 *   3. Anexa PDF final governado (POST /trainings/:id/pdf/file)
 *   4. Confere acesso ao PDF (GET /trainings/:id/pdf) e estado do banco
 *      (trainings + document_registry com hash)
 *
 * Observação: o módulo Treinamento não possui portal de validação pública com grant
...
const SMOKE_USER_NAME = 'K6 TESTE GANDRA TRAINING CONTROLADO';
const SMOKE_USER_EMAIL = 'k6.gandra.training.smoke@invalid.local';
const SMOKE_USER_FUNCTION = 'TST Smoke Controlado';
const SMOKE_USER_BASE_CPF = '987654311';
const UA = 'sgs-prod-gandra-training-smoke/1.0';
...
function buildSmokeTitulo() {
  ...
  return `${TEST_DOCUMENT_PREFIX}${stamp}-TRN-001`;
}
...
async function findResumableDid(client, titulo) {
  const rows = await client.query(
    `SELECT id, nome, pdf_file_key
       FROM trainings
      WHERE company_id = $1 AND deleted_at IS NULL AND nome LIKE $2
  ...
  if (rows.rows.length >= MAX_TEST_DOCUMENTS) {
    ...
    // nome tenha sido gerado em outro dia (o smoke é resumível).
    if (rows.rows.length === 1) return rows.rows[0];
    throw new Error(
      `Já existe ${rows.rows.length} treinamento de teste com prefixo ${TEST_DOCUMENT_PREFIX}. Abortando.`,
    );
  }
  return rows.rows.find((row) => row.nome === titulo) || null;
}
...
async function buildSmokePdfBuffer(params) {
  ...
  page.drawText('SGS - Treinamento Smoke Test Controlado', {
    x: 52,
    y: 788,
    size: 18,
    font: bold,
  ...
  const lines = [
    ...
    `Treinamento ID: ${params.trainingId}`,
...
async function attachPdf(accessToken, trainingId, pdfBuffer, fileName) {
  const csrfHeaders = await getMutationCsrfHeaders();
  const form = new FormData();
  form.set('file', new File([pdfBuffer], fileName, { type: 'application/pdf' }));
  const response = await fetch(`${API_BASE_URL}/trainings/${trainingId}/pdf/file`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'User-Agent': UA,
...
async function verifyDatabaseState(client, trainingId) {
  const trainingRes = await client.query(
    `SELECT d.id, d.nome, d.company_id, d.pdf_file_key,
  ...
  if (trainingRes.rows.length !== 1)
    throw new Error('Treinamento criado não encontrado para verificação final.');
  ...
  const registryRes = await client.query(
    `SELECT id, company_id, module, entity_id, document_code, file_key,
      ...
      WHERE module = 'training' AND entity_id = $1
      ORDER BY created_at DESC`,
    [trainingId],
  );

  const cat = trainingRes.rows[0];
  if (cat.company_id !== TEST_COMPANY_ID)
    throw new Error('Treinamento persistido fora do tenant esperado.');
  if (cat.user_company_id !== TEST_COMPANY_ID)
    throw new Error('Usuário do treinamento não pertence ao tenant esperado.');
  if (registryRes.rows.length !== 1)
    throw new Error(
      `Esperado 1 registro governado para o treinamento; encontrado ${registryRes.rows.length}.`,
    );
  if (registryRes.rows[0].company_id !== TEST_COMPANY_ID)
    throw new Error('Registro governado pertence a tenant incorreto.');

  return { training: cat, registry: registryRes.rows[0] };
}
...
async function run() {
  ...
  try {
    ...
    if (resumable)
      warnings.push(
        `resuming_existing_training:${maskId(resumable.id)} status=${resumable.status}`,
      );
    ...
    // 1. Criação
    let trainingId = resumable?.id || null;
    let currentStatus = resumable?.status || null;
    let hasPdfAlready = Boolean(resumable?.pdf_file_key);

    if (!trainingId) {
      const created = await requestJson('/trainings', session.accessToken, {
        method: 'POST',
        companyId: TEST_COMPANY_ID,
        includeCsrf: true,
        ...
        body: JSON.stringify({
          ...
          data_vencimento: new Date(Date.now() + 365 * 86400000)
            .toISOString()
            .slice(0, 10),
      ...
      if (!created.ok || !created.body?.id) {
        throw new Error(
          `Criação do treinamento falhou. status=${created.status} body=${JSON.stringify(created.body)}`,
        );
      }
      trainingId = created.body.id;
    ...
    const db = await verifyDatabaseState(client, trainingId);
    ...
    const report = {
      ...
      database: {
        trainingId: maskId(db.training.id),

        trainingCompanyId: maskId(db.training.company_id),
        userCompanyId: maskId(db.training.user_company_id),
        hasPdfKey: Boolean(db.training.pdf_file_key),
      ...
      publicValidation: {
        status: null,
        valid: null,
        note: 'not_applicable: módulo Treinamento não possui portal de validação pública com grant.',
      },
      artifacts: {
