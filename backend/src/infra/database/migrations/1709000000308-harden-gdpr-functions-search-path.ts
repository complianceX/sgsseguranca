...
export class HardenGdprFunctionsSearchPath1709000000308 implements MigrationInterface {
  name = 'HardenGdprFunctionsSearchPath1709000000308';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
          RETURN QUERY SELECT 'mail_logs'::text, 0;
        END;
        ...
        EXCEPTION WHEN OTHERS THEN
          RETURN QUERY SELECT 'user_sessions'::text, 0;
        END;
        ...
        EXCEPTION WHEN OTHERS THEN
          RETURN QUERY SELECT 'forensic_trail_events'::text, 0;
        END;
        ...
        EXCEPTION WHEN OTHERS THEN
          RETURN QUERY SELECT 'activities'::text, 0;
        END;
        ...
        EXCEPTION WHEN OTHERS THEN
          RETURN QUERY SELECT 'audit_logs'::text, 0;
        END;
        ...
        EXCEPTION WHEN OTHERS THEN
          RETURN QUERY SELECT 'ai_interactions'::text, 0;
        END;
      END;
      $$ LANGUAGE plpgsql;
    `);
    ...
    await queryRunner.query(`
      ...
      BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
          RETURN QUERY SELECT 'document_registry'::text, 0;
        END;
        ...
        EXCEPTION WHEN OTHERS THEN
          RETURN QUERY SELECT 'user_consents'::text, 0;
        END;
      END;
      $$ LANGUAGE plpgsql;
    `);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      BEGIN
        ...
        IF EXISTS (
          SELECT 1 FROM information_schema.columns cols
          WHERE cols.table_schema = 'public' AND cols.table_name = 'forensic_trail_events' AND cols.column_name = 'occurred_at'
        ) THEN
          DELETE FROM forensic_trail_events WHERE occurred_at < NOW() - INTERVAL '2 years';
          GET DIAGNOSTICS v_count = ROW_COUNT;
        ELSIF EXISTS (
          SELECT 1 FROM information_schema.columns cols
          WHERE cols.table_schema = 'public' AND cols.table_name = 'forensic_trail_events' AND cols.column_name = 'created_at'
        ) THEN
          DELETE FROM forensic_trail_events WHERE created_at < NOW() - INTERVAL '2 years';
          GET DIAGNOSTICS v_count = ROW_COUNT;
        ELSE
          v_count := 0;
        END IF;
        ...
        IF EXISTS (
          SELECT 1 FROM information_schema.columns cols
          WHERE cols.table_schema = 'public' AND cols.table_name = 'activities' AND cols.column_name = 'deleted_at'
        ) THEN
        ...
        IF EXISTS (
          SELECT 1 FROM information_schema.columns cols
          WHERE cols.table_schema = 'public' AND cols.table_name = 'audit_logs' AND cols.column_name = 'timestamp'
        ) THEN
          DELETE FROM audit_logs WHERE "timestamp" < NOW() - INTERVAL '2 years';
          GET DIAGNOSTICS v_count = ROW_COUNT;
        ELSIF EXISTS (
          SELECT 1 FROM information_schema.columns cols
          WHERE cols.table_schema = 'public' AND cols.table_name = 'audit_logs' AND cols.column_name = 'created_at'
        ) THEN
        ...
        IF EXISTS (
          SELECT 1 FROM information_schema.columns cols
          WHERE cols.table_schema = 'public' AND cols.table_name = 'ai_interactions' AND cols.column_name = 'deleted_at'
        ) AND EXISTS (
          SELECT 1 FROM information_schema.columns cols
          WHERE cols.table_schema = 'public' AND cols.table_name = 'ai_interactions' AND cols.column_name = 'created_at'
        ) THEN
          DELETE FROM ai_interactions WHERE COALESCE(deleted_at, created_at) < NOW() - INTERVAL '1 year';
          GET DIAGNOSTICS v_count = ROW_COUNT;
        ELSE
          v_count := 0;
        END IF;
    ...
    await queryRunner.query(`
      ...
      BEGIN
        ...
        IF to_regclass('public.audit_logs') IS NOT NULL THEN
          IF EXISTS (
            SELECT 1 FROM information_schema.columns cols
            WHERE cols.table_schema = 'public' AND cols.table_name = 'audit_logs' AND cols.column_name = 'deleted_at'
          ) THEN
        ...
        IF to_regclass('public.document_registry') IS NOT NULL THEN
          IF EXISTS (
            SELECT 1 FROM information_schema.columns cols
            WHERE cols.table_schema = 'public' AND cols.table_name = 'document_registry' AND cols.column_name = 'deleted_at'
          ) THEN
            UPDATE document_registry SET deleted_at = NOW(), created_by = NULL WHERE created_by = p_user_id AND deleted_at IS NULL;
          ELSE
