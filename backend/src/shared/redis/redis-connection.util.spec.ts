...
describe('redis-connection.util', () => {
  ...
  it('resolve tier RATE_LIMIT a partir de REDIS_RATE_LIMIT_URL', () => {
    const connection = resolveRedisConnection(
      {
        REDIS_RATE_LIMIT_URL:
          'rediss://rate-user:rate-secret@rate.redis.local:6382',
      },
      'rateLimit',
    );
    ...
    expect(connection).toEqual({
      source: 'url',
      url: 'rediss://rate-user:rate-secret@rate.redis.local:6382',
  ...
  it('aceita Redis remoto com TLS em produção', () => {
    expect(() =>
      assertSecureRedisConnection(
        {
          source: 'url',
          url: 'redis://default:secret@redis.example.com:6380',
          host: 'redis.example.com',
          port: 6380,
          password: 'secret',
          tls: { rejectUnauthorized: true },
